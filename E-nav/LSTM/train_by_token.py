...
train_x_placeholder = tf.placeholder(tf.float32, shape = [None, 50, 300])
...
with tf.variable_scope(layer_name, reuse=tf.AUTO_REUSE):
    ...
    if is_training:
        outputs = tf.nn.dropout(outputs, keep_prob=params.dropout_prob)
...
with tf.Session() as sess :
    ...
    for epoch in t :
        for step in range(max_step) :
            ...
            if acc_val > best_acc :
                best_acc = acc_val
                best_saver.save(sess, os.path.join(os.getcwd(), 'model', 'doc_based'))
            t.set_postfix(best_acc = best_acc, loss = loss_val)
